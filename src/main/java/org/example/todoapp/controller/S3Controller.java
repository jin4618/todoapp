...
//    @PostMapping("/upload")
//    public ResponseEntity<String> uploadFile(@RequestParam("file") MultipartFile file) {
//        try {   // throws IOException 처리
//            s3Service.uploadFile(file);
//            return ResponseEntity.ok("File uploaded successfully" + file.getOriginalFilename());
//
//        } catch (Exception e) {
//            return ResponseEntity.status(500).body("File uploaded fail :: " + e.getMessage());
//        }
//    }
//
...
//                byte[] buffer = new byte[1024];
//                int bytesRead;
//                while ((bytesRead = inputStream.read(buffer)) != -1) {  // -1 : end of file
//                    outputStream.write(buffer, 0, bytesRead);
//                }
//                inputStream.close();
//            };
//            return ResponseEntity.ok()
//                    .contentType(MediaType.APPLICATION_OCTET_STREAM)
//                    .header(HttpHeaders.CONTENT_DISPOSITION, "attachement; filename\"" + key + "\"")
//                    .body(responseBody);
//
//        } catch (Exception e) {
//            return ResponseEntity.status(500).body(null);
